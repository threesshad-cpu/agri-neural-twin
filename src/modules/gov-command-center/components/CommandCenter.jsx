...
export default function CommandCenter({ onDistrictSelect }) {
    ...
    useEffect(() => {
        dataService.getDistrictsList().then(data => {
            ...
            const runoffPrevented = ((riskCount * 18.4) + 42.6).toFixed(1);

            setStats({
                yield: totalYield.toLocaleString(),
                density: density.toFixed(1),
                gdp: gdpShielded.toLocaleString(),
                runoffPrevented: runoffPrevented
            });
        });
    }, []);
    ...
    const getRiskLevel = (risk) => {
        const HIGH = ['Severe Drought', 'Flooding', 'Flash Floods', 'Cyclone Vulnerability', 'Landslides', 'Industrial Pollution'];
        const LOW = ['Low'];
        if (LOW.includes(risk)) return 'low';
        if (HIGH.includes(risk)) return 'high';
        return 'medium';
    };
    ...
    return (
        <div style={{ padding: '1rem', maxWidth: '100%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '2rem', marginBottom: '3rem' }}>
                ...
                <div className="gov-card" style={{ padding: '1.5rem', borderLeft: '5px solid #0891B2' }}>
                    <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#0E7490', fontFamily: 'var(--font-sans)', lineHeight: '1' }}>{stats.runoffPrevented} T</div>
                    <div style={{ textTransform: 'uppercase', fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: '0.5rem', fontWeight: '600' }}>{t('nutrient_recovery_page.runoff_prevented', 'N-P-K Runoff Prevented')}</div>
                </div>
            </div>
